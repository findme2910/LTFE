...
const MovieSchedule: React.FC = () => {
   ...
   return (
      <div className='container mx-auto my-8'>
         ...
         <div className='mb-4'>
            <select
               className='p-2 bg-transparent mr-2 border rounded outline-none'
               value={selectedCity}
               onChange={handleCityChange}
            >
               {cities.map((city, index) => (
                  <option className='bg-primary-foreground' key={index} value={city.value}>
                     {city.label}
                  </option>
               ))}
            </select>
            ...
            >
               {getCinemas(selectedCity).map((cinema, index) => (
                  <option className='bg-primary-foreground' key={index} value={cinema.value}>
                     {cinema.label}
                  </option>
               ))}
            </select>
               ...
               <table className='min-w-full  border  mt-6'>
                  <thead>
                     <tr>
                        ...
                        <th className='py-2 px-4 border-b'>Giờ chiếu</th>
